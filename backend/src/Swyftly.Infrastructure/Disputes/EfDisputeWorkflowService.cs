    ...
    IAuditLogService auditLogService) : IDisputeWorkflowService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (string.Equals(actorRole, SwyftlyRoles.Buyer, StringComparison.OrdinalIgnoreCase))
        {
            query = query.Where(dispute => dispute.BuyerId == actorProfileId);
        }
        else if (string.Equals(actorRole, SwyftlyRoles.Seller, StringComparison.OrdinalIgnoreCase))
        {
