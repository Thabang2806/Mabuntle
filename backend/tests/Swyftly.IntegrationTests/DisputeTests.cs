...
public sealed class DisputeTests
{
    ...
    public async Task BuyerCanOpenDispute_SellerCanRespond_AdminCanResolveSellerFavouredAndReleasePayout()
    {
        ...
        var sellerAuth = await RegisterAndLoginAsync(sellerClient, "seller-dispute@example.test", SwyftlyRoles.Seller);
        var buyerAuth = await RegisterAndLoginAsync(buyerClient, "buyer-dispute@example.test", SwyftlyRoles.Buyer);
    ...
    public async Task AdminCanResolveBuyerFavoured_CreatesRefundRequestAndKeepsPayoutHeld()
    {
        ...
        var buyerAuth = await RegisterAndLoginAsync(buyerClient, "buyer-favoured-dispute@example.test", SwyftlyRoles.Buyer);
    ...
    {
        private readonly string _databaseName = $"SwyftlyDisputeTests_{Guid.NewGuid():N}";
