...
public sealed class SellerAdCampaignTests
{
    ...
    public async Task VerifiedSeller_CanCreateDraftCampaign_AndSubmitForReview()
    {
        ...
        var auth = await RegisterAndLoginAsync(client, "ads-seller@example.test", SwyftlyRoles.Seller);
    ...
    public async Task Seller_CannotCreateCampaignForIneligibleProduct()
    {
        using var factory = new SellerAdCampaignTestFactory();
        using var client = factory.CreateClient();
        var auth = await RegisterAndLoginAsync(client, "ads-ineligible-seller@example.test", SwyftlyRoles.Seller);
    ...
    public async Task Seller_CannotReadAnotherSellersCampaign()
    {
        ...
        var sellerOneAuth = await RegisterAndLoginAsync(sellerOneClient, "ads-owner@example.test", SwyftlyRoles.Seller);
        var sellerTwoAuth = await RegisterAndLoginAsync(sellerTwoClient, "ads-other@example.test", SwyftlyRoles.Seller);
    ...
    {
        private readonly string _databaseName = $"SwyftlySellerAdCampaignTests_{Guid.NewGuid():N}";
