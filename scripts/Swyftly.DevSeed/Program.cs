...
services.AddDbContext<SwyftlyDbContext>(builder =>
{
...
var financeOperator = await EnsureUserAsync(
    userManager,
    "finance.operator@swyftly.local",
    options.Password,
    options.ResetPasswords,
...
var financeApprover = await EnsureUserAsync(
    userManager,
    "finance.approver@swyftly.local",
    options.Password,
    options.ResetPasswords,
...
Console.WriteLine("Swyftly development users are ready.");
Console.WriteLine();
Console.WriteLine("Email                         Roles");
Console.WriteLine("admin@swyftly.local           SuperAdmin, Admin");
Console.WriteLine("finance.operator@swyftly.local FinanceOperator");
Console.WriteLine("finance.approver@swyftly.local FinanceApprover");
Console.WriteLine("support@swyftly.local         SupportAgent");
Console.WriteLine("buyer@swyftly.local           Buyer");
Console.WriteLine("seller@swyftly.local          Seller; verified seller profile; published storefront");
if (options.SeedSellerFlowDemo)
{
    Console.WriteLine("seller.pending@swyftly.local  Seller; complete onboarding; pending admin review");
}
...
{
    foreach (var role in SwyftlyRoles.All)
    {
...
static async Task<BuyerProfile> EnsureBuyerProfileAsync(SwyftlyDbContext dbContext, Guid userId)
{
...
static async Task<SellerProfile> EnsureVerifiedSellerProfileAsync(SwyftlyDbContext dbContext, Guid userId, Guid adminUserId)
{
...
static async Task EnsureBuyerDemoAddressAsync(SwyftlyDbContext dbContext, Guid buyerId)
{
...
static async Task EnsureSampleProductsAsync(SwyftlyDbContext dbContext, Guid sellerId)
{
...
static async Task<SellerProfile> EnsurePendingSellerProfileAsync(SwyftlyDbContext dbContext, Guid userId)
{
    var now = DateTimeOffset.UtcNow;
    var seller = await dbContext.SellerProfiles.SingleOrDefaultAsync(item => item.UserId == userId);
    if (seller is null)
...
static async Task<Product> EnsurePendingReviewProductAsync(SwyftlyDbContext dbContext, Guid sellerId)
{
...
static async Task EnsurePendingAdCampaignAsync(SwyftlyDbContext dbContext, Guid sellerId, Guid productId)
{
    var now = DateTimeOffset.UtcNow;
...
static async Task EnsureDemoAdBudgetAsync(SwyftlyDbContext dbContext, Guid campaignId, DateTimeOffset now)
{
...
static void ResetProductStatusForDemo(SwyftlyDbContext dbContext, Product product, ProductStatus status)
{
...
static async Task EnsureSeedImageAsync(SwyftlyDbContext dbContext, Guid productId, SampleProductSeed seed)
{
...
static async Task<string> CreateUniqueStorefrontSlugAsync(SwyftlyDbContext dbContext, Guid sellerId)
{
    const string baseSlug = "swyftly-dev-store";
    ...
    return $"swyftly-dev-store-{sellerId.ToString("N")[..8]}";
}
    ...
    bool SeedSellerFlowDemo)
{
    ...
    private static void PrintUsage()
    {
        ...
        Console.WriteLine("  dotnet run --project scripts/Swyftly.DevSeed -- --connection \"Host=...\" --password \"Your_dev_password1!\"");
        Console.WriteLine();
