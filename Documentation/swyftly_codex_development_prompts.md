# Swyftly Codex Development Prompt Pack
...
## 1. What I think of the name “Swyftly”

**Swyftly** is a strong marketplace name. It suggests speed, convenience, and smooth transactions, which works well for a fashion and beauty ecommerce marketplace. The spelling gives it a modern startup feel and makes it more brandable than the plain word “Swiftly.”
...
- **Swyftly — Fashion moves faster here.**
- **Swyftly — Shop style, beauty, and accessories from local sellers.**
- **Swyftly — Discover it. Love it. Buy it.**
- **Swyftly — The marketplace for fast fashion finds and beauty essentials.**
...
For product UI, I would use the brand name consistently as **Swyftly**, not **SWYFTLY**, because the softer title case fits fashion and beauty better.
...
```text
You are helping build Swyftly, a transactional ecommerce marketplace for fashion, clothing, jewellery, accessories, and beauty products.
...
```text
Context:
We are building Swyftly, a transactional ecommerce marketplace for fashion, clothing, jewellery, accessories, and beauty products.
...
/swyftly
  ...
  /backend
    /src
      ...
      Swyftly.Application
      Swyftly.Domain
      Swyftly.Infrastructure
      Swyftly.Worker
    /tests
      Swyftly.UnitTests
      Swyftly.IntegrationTests
  /frontend
    /swyftly-web
  /database
    /migrations
...
- Create an Angular application under /frontend/swyftly-web.
...
```text
Goal:
Create or update AGENTS.md for the Swyftly repository.
...
```text
Context:
The monorepo exists for Swyftly.
...
- Use the Swyftly colour palette:
  - Primary: #3A1D32
...
```text
Goal:
Create docs/architecture.md for Swyftly.
...
```text
Goal:
Create docs/feature-roadmap.md for Swyftly.
...
- Keep the roadmap specific to Swyftly.
```
...
```text
Goal:
Create docs/coding-standards.md for Swyftly.
...
```text
Context:
Swyftly backend projects exist.

Goal:
Configure PostgreSQL with Entity Framework Core.

Requirements:
- Add EF Core and Npgsql packages to Swyftly.Infrastructure.
- Create SwyftlyDbContext.
- Configure dependency injection in Swyftly.Api.
...
```text
Goal:
Add common backend building blocks for Swyftly.
...
```text
Goal:
Add GitHub Actions CI for Swyftly.
...
```text
Context:
Swyftly is a transactional ecommerce marketplace.
...
```text
Goal:
Create Angular seller onboarding wizard for Swyftly.
...
```text
Goal:
Implement Swyftly category and category attribute system.
...
```text
Goal:
Implement product aggregate for Swyftly.
...
```text
Goal:
Implement product image records and upload placeholder for Swyftly.
...
```text
Goal:
Create the database schema and DTOs for Swyftly's AI Fashion Product Listing Assistant.
...
```text
Context:
Swyftly has product drafts, categories, category attributes, and AI suggestion tables.
...
```text
Goal:
Build Angular UI for Swyftly's AI Fashion Product Listing Assistant inside the seller product form.
...
```text
Goal:
Implement basic AI/business-rule moderation for Swyftly product listings.
...
```text
Goal:
Create buyer-facing shop and category pages for Swyftly.
...
```text
Goal:
Prepare Swyftly for dedicated search engine integration.
...
```text
Goal:
Prepare Swyftly for AI semantic search and similar products using pgvector.
...
```text
Goal:
Create Angular cart and checkout UI for Swyftly MVP.
...
```text
Goal:
Create payment provider abstraction for Swyftly.
...
```text
Goal:
Implement Swyftly internal ledger.
...
```text
Goal:
Implement return request workflow for Swyftly.
...
```text
Goal:
Implement dispute workflow for Swyftly.
...
```text
Goal:
Create admin dashboard foundation for Swyftly.
...
```text
Goal:
Implement seller ad campaign domain model for Swyftly.

Context:
Swyftly MVP is free for sellers to join and list products. Revenue comes from transaction fees first. Later, sellers can pay for promoted listings and ad campaigns.
...
```text
Goal:
Implement ad tracking for Swyftly promoted listings.
...
```text
Goal:
Create AI usage analytics for Swyftly.
...
```text
Goal:
Review and harden authorization across Swyftly backend.
...
```text
Goal:
Add observability foundation to Swyftly backend.
...
```text
Goal:
Implement buyer AI shopping assistant that recommends real Swyftly products only.
...
```text
Goal:
Implement MVP visual search for Swyftly.
...
```text
Review this Swyftly pull request as a senior full-stack engineer.
...
```text
Perform a focused security review of the current Swyftly codebase.
...
```text
Review Swyftly's payment, ledger, seller balance, refund, and payout logic.
...
```text
Refactor the specified Swyftly module without changing behavior.
...
```text
Update Swyftly documentation after the latest code changes.
...
```text
You are working on Swyftly, a transactional ecommerce marketplace for fashion, clothing, jewellery, accessories, and beauty products.
