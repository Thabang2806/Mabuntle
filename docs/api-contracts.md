...
Production CORS is configured with `Cors__AllowedOrigins__*`. Values must be explicit external HTTPS frontend origins such as `https://mabuntle.com`, `https://seller.mabuntle.com`, and `https://admin.mabuntle.com`; wildcard, localhost, and placeholder origins are rejected in production because authenticated API calls and SignalR use credentials. Localhost origins remain available outside production for Angular development.
