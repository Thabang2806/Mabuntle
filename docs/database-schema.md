...
- `audit_logs`, currently used for admin seller approval/rejection/suspension actions, seller payout-profile change submission/review actions, admin product approval/rejection/change-request actions, admin review moderation actions, seller inventory adjustments, seller delivery-method create/update/activate/deactivate actions, admin pickup-point create/update/activate/deactivate actions, and admin category/attribute create/update/activate/deactivate actions.
...
- `notifications`, linked to Identity users as recipients, storing notification type, title, message, related entity metadata, read timestamp, in-app visibility flag, and creation timestamp. Buyer notification creation checks category preferences for known buyer notification types. Buyer real-time SignalR delivery uses these persisted rows as the source of truth and does not add storage tables.
...
Orders start as `PendingPayment`. Payment initiation, provider webhook events, payment reconciliation review evidence, stored checkout URLs for retry, successful-payment ledger entries, paid-cart cleanup, seller pending-balance crediting, seller payout records, seller payout-profile change re-verification, conditional inventory reservation holds, reservation confirmation on payment success, reservation release on payment failure, seller manual fulfilment actions, ready-to-ship, provider-neutral carrier booking/tracking with Manual/Fake providers, carrier provider comparison notes, seller delivery confirmation, delivery-failed/returned-to-sender exception tracking, buyer shipment tracking, return requests, seller return responses, verified-buyer reviews, buyer review moderation, buyer wishlist items, buyer profile settings, saved delivery addresses, local address verification, delivery instructions, seller-managed delivery methods/rates including pickup delivery, order delivery-address verification, delivery-method, and pickup-point snapshots, in-app notifications, buyer transactional email notification outbox delivery, buyer real-time notification delivery over SignalR, notification preferences, notification wiring for review/return/order/support/delivery workflows, refund records, provider refund abstraction calls, ledger reversal entries, seller-balance refund debits, payout item adjustments, fake-provider payout lifecycle transitions, standalone disputes, dispute messages/evidence, dispute-triggered payout holds, buyer-favoured dispute refund request creation, admin dispute resolution, support tickets with private internal notes, seller ad campaign draft/review-submission/admin-review persistence, product listing revisions for published content changes, media asset metadata/variants/cleanup, ad event tracking, and paid-order ad attribution now exist. Real external payout provider integration, provider status-query settlement, external address verification/geocoding, pickup-network APIs, carrier-provided rate calculation, and real carrier-provider integration are future work.
