...
- `refresh_tokens`, linked to Identity users and storing hashed refresh tokens only.
- `audit_logs`, currently used for admin seller approval/rejection/suspension actions and admin product approval/rejection/change-request actions.
...
- `product_variants`, linked to products, with SKU, size, colour, price, compare-at price, stock, reserved quantity, barcode, and Active/Inactive/OutOfStock status.
...
- `inventory_reservations`, linked to product variants, buyer profiles, and carts, storing checkout inventory holds with Active/Confirmed/Expired/Cancelled status and expiry timestamps.
...
- `payments`, linked to orders and buyer profiles, storing local payment records, configured provider name/reference, amount, currency, status, and paid/failed timestamps.
- `payment_events`, linked optionally to payments, storing raw provider webhook JSON, provider event id, event type, received/processed timestamps, processing status, and error details. `(Provider, ProviderEventId)` is unique for webhook idempotency.
- `refunds`, linked to orders, payments, buyers, sellers, and optionally return requests, storing requested/approved/processing/refunded/failed/rejected status, amount, provider refund reference, reason, and audit timestamps.
...
- `seller_payouts`, linked to sellers, storing payout amount, currency, Pending/OnHold/Available/Processing/PaidOut/Reversed/Failed status, hold/release audit details, and timestamps.
- `seller_payout_items`, linked to seller payouts and source ledger entries, tying payout records back to order/payment ledger activity.
...
Orders start as `PendingPayment`. Payment initiation, provider webhook events, successful-payment ledger entries, seller pending-balance crediting, seller payout records, reservation confirmation on payment success, reservation release on payment failure, seller manual fulfilment actions, buyer shipment tracking, return requests, seller return responses, refund records, provider refund abstraction calls, ledger reversal entries, seller-balance refund debits, standalone disputes, dispute messages/evidence, dispute-triggered payout holds, admin dispute resolution, support tickets with private internal notes, seller ad campaign draft/review-submission/admin-review persistence, ad event tracking, and paid-order ad attribution now exist. Delivery-confirmation workflow, payout processing/release to external providers, full balance availability transitions, and production payment reconciliation are future work.

## Planned Modules

- Delivery confirmation and carrier integration.
- External payout processing and full seller balance availability transitions.
- Payment reconciliation hardening for duplicate initiation, refund idempotency, and out-of-order webhooks.
- Reviews, wishlist, notifications, and broader admin audit workflows.
