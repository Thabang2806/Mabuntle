...
Latest buyer-audit note: Phase 10X ran seeded buyer route/API smoke, fixed the cart lifecycle blocker that left checked-out carts active and prevented clean new carts, and recorded next buyer priorities in `docs/buyer-flow-gap-audit-results.md`. Phase 10Y then improved checkout confidence with stateful checkout progress, stale manual-address quote clearing, buyer-safe payment summaries, result-page refresh/polling, sanitized failed-payment reasons, and moderation-accurate review copy. Phase 10Z implemented catalog discovery/product confidence on existing public APIs with removable shop filter chips, category filters, storefront filtering, product confidence copy, and same-category related products while deferring backend facet metadata, storefront pagination, seller rating aggregation, and a dedicated similar-products endpoint. Phase 11A added a local API helper for paid-and-delivered buyer order context. Phase 11B added post-purchase clarity polish for order-to-support handoff, support query prefill, return quantity caps, dispute empty-state copy, and buyer returns list test coverage. Phase 11C completed desktop/mobile browser route sign-off for the delivered-order post-purchase pages and fixed stale support-query context plus return-detail support handoff links. Phase 11D completed the buyer account trust polish/action sign-off after following the planning-agent workflow, including delivered-order return creation, order-linked support-ticket creation, notification Open actions, account next-action prompts, buyer-safe settings copy, and wishlist/support trust copy. Phase 11F completed refund-state browser sign-off, Phase 11G implemented buyer growth depth for `/assistant` and `/visual-search` with browser-local recents and `/shop` query-param handoff, Phase 11H added privacy-conscious buyer AI discovery telemetry plus aggregate admin reporting without persisted raw prompts/images, Phase 11I added opt-in server-side AI discovery history with privacy controls and `/account/ai-history`, Phase 11J added aggregate-only AI discovery outcome attribution for cart, checkout, order, and paid-order reporting, Phase 11K added an API-only attribution QA helper plus clearer attributed-open report labeling, and Phase 11L added opt-in, explainable AI discovery personalization from safe shopping/history summaries only.
...
| B-03 | Product Listing/Search Results | `/shop`, `ShopPageComponent` | Partial | Search, filters, sort, pagination, product grid, removable active filter chips, quick refinements, and product-card confidence metadata exist. Server-computed facet/count metadata remains deferred. |
| B-04 | Product Detail Page | `/product/:slug`, `ProductDetailPageComponent` | Partial | Product detail, variant selection, attributes, price, seller link, wishlist save/remove, add-to-cart, public review reads, policy/care/disclaimer snippets, out-of-stock action copy, and same-category related products exist. A dedicated similar-products endpoint remains deferred. |
| B-05 | Seller Storefront | `/seller/:storeSlug`, `SellerStorefrontPageComponent` | Partial | Hero, product grid, seller trust/policy snippets, in-stock count, and client-side storefront search/availability/sort filters exist. Backend storefront product pagination and first-class seller rating aggregation remain deferred. |
...
| B-07 | Cart | `/cart`, `CartPageComponent` | Partial | Cart item update/remove, single-seller checkout, variant/SKU detail, product image metadata/fallback visuals, price clarity, trust messaging, product detail links, save-for-later, and checkout delivery-rate handoff are present. Order creation now checks out the source cart so buyers can start a clean new cart after checkout. Richer checkout state remains future work. |
| B-08 | Checkout Address & Delivery | `/checkout`, `CheckoutPageComponent`; `/account/settings`, `BuyerSettingsPageComponent` | Partial | Checkout can use saved delivery addresses or an inline one-off address with optional delivery instructions, runs warning-only local address verification, loads seller-managed delivery methods/rates, supports pickup-point choices for pickup methods, requires a selected delivery option, clears quoted delivery when one-off address fields change, and orders persist delivery-address verification plus delivery-method/rate/pickup snapshots. External address verification/geocoding and carrier-provided rates remain future work. |
...
| B-10 | Checkout Payment | `/checkout`, `CheckoutPageComponent` | Partial | Checkout now creates a pending order, initiates backend payment, redirects to the provider checkout URL, shows stateful address/delivery/payment progress, and carries a sanitized payment-start failure reason to `/checkout/failed`. Real provider SDK/UI and saved payment methods remain future work. |
| B-11 | Payment Failed | `/checkout/failed`, `CheckoutFailedPageComponent` | Partial | Route loads order context when available, renders buyer-safe payment summary, polls briefly while pending, supports pending-payment retry, shows sanitized payment-start errors, and tells buyers to restart checkout for cancelled orders. Change-payment details await real payment provider UI. |
| B-12 | Order Confirmation | `/checkout/success`, `CheckoutSuccessPageComponent` | Partial | Route loads order context, renders buyer-safe payment summary, shows webhook-confirmed order status, supports pending-payment retry and manual refresh, and polls briefly while payment remains pending. Rich paid-order confirmation and delivery ETA remain future polish. |
| B-13 | Buyer Account Dashboard | `/account`, `AccountPageComponent` | Partial | Dashboard now summarizes orders, active returns, open disputes, support tickets, wishlist, reviews, notifications, and refund next-action prompts using existing buyer APIs. `/account/settings` manages lightweight profile details, saved delivery addresses with verification previews/status, separate in-app/email notification preferences, and opt-in AI discovery history privacy controls. Buyer real-time notification badge/toast exists. SMS and push remain future work. |
...
| B-15 | Buyer Order Detail | `/account/orders/:orderId`, `BuyerOrderDetailPageComponent` | Partial | API-backed detail shows items, totals, buyer-safe latest payment summary with refresh/retry context, delivery-address/instruction/verification, selected delivery-method and pickup-point snapshots, status history, shipment event timelines, carrier status metadata, delivery exception support CTAs, delivered-order return/review actions, and moderation-accurate review copy. Real carrier-provider integration remains future work. |
| B-16 | Return/Refund Request | `/account/returns`, `/account/returns/:returnRequestId`, `/account/refunds` | Partial | API-backed buyer return list/detail, rejected-return dispute escalation, buyer-safe refund list/detail reads, and refund panels on order/return detail exist. Phase 11F browser sign-off verified a return-linked refund created and approved through existing finance workflows on desktop and mobile. Buyers can view refund state only; refund creation/approval remains admin/finance-controlled. |
...
| B-19 | AI Shopping Assistant | `/assistant`, `BuyerAiAssistantPageComponent`; `/account/ai-history` | Later/Beta | Route exists and calls buyer AI services. Phase 11G added browser-local recent prompts, clear history, confidence explanations for current recommendations, clearer product actions, and `/shop` query-param handoff. Phase 11H added structured usefulness feedback and sanitized event telemetry for search, product-open, and shop-handoff actions. Phase 11I added opt-in server-side safe history summaries and privacy controls without storing raw prompts. Phase 11J added aggregate-only outcome attribution from AI discovery to cart, checkout, order, and paid-order outcomes. Phase 11L added opt-in, explainable personalization from wishlist, cart/order interest, and enabled AI history summaries only. |
| B-20 | Visual Search Upload | `/visual-search`, `BuyerVisualSearchPageComponent`; `/account/ai-history` | Later/Beta | Route exists. Phase 11G added browser-local recent text references, reset/remove controls, confidence bands, stronger image-retention/warning copy, and `/shop` query-param handoff. Phase 11H added structured usefulness feedback and sanitized event telemetry while continuing not to store uploaded image data, previews, or base64 content. Phase 11I added opt-in safe server-side history summaries without storing uploaded images, previews, base64, or provider payloads. Phase 11J added aggregate-only outcome attribution for AI-assisted shopping actions, and Phase 11L added opt-in personalized result labels/reordering without changing AI provider calls. Richer similarity diagnostics remain deferred. |
...
7. **Buyer trust is underdeveloped.** Checkout, product detail, seller storefronts, returns, and payment states need visible trust cues: verified seller, secure checkout copy, return policy, delivery expectations, support path, price/fee clarity, and stronger post-purchase state confidence.
...
Status: Phase 11G implemented buyer growth depth for `/assistant` and `/visual-search`. The implemented depth includes browser-local recent prompts/references, confidence explanations tied to the current prompt or visual reference, and `/shop` handoff via query parameters. Phase 11H added buyer-authenticated, privacy-conscious AI discovery telemetry and aggregate admin reporting for search/product-open/shop-handoff/feedback events without storing prompts, uploaded images, provider payloads, or free-text feedback. Phase 11I added opt-in server-side safe AI discovery history summaries under `/account/ai-history` plus privacy controls in `/account/settings`. Phase 11J added aggregate-only outcome attribution from AI discovery to product-open, cart-add, checkout-start, order-created, and signed-webhook-paid outcomes. Phase 11K added a repeatable helper to create attributed assistant/visual-search paid-order evidence through existing APIs and clarified attributed-open report labels. Phase 11L added opt-in, explainable AI discovery personalization using only safe buyer-owned shopping/history summaries. Deferred growth gaps include richer ranking/similarity diagnostics.
