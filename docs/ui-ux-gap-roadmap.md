...
Latest seller-notification note: Phase 10A added seller-managed notification preferences for Verification, Products, Revisions, and Ads, and widened the existing SignalR notification hub so sellers get realtime unread badges, toasts, and `/seller/notifications` sync after visible notification persistence.
...
Latest seller-QA note: Phase 10F ran the seeded seller lifecycle QA path twice, verified backend/frontend builds and tests, added `docs/seller-flow-qa-results.md`, and fixed the verified seller dashboard update row so support, notification, and ad-review paths are clearer.
...
Latest seller-product note: Phase 9W added moderation-aware published variant and pricing revisions. Sellers can stage SKU, size, colour, price, compare-at price, barcode, new variants, and variant deactivation from the published product editor, while admins review those changes from the product moderation queue before live variants and active cart snapshots are updated.
...
| S-21 | Seller Analytics | `/seller/analytics`, `SellerAnalyticsPageComponent` | Partial | Seller analytics now include API-backed date filters, daily/weekly sales trend buckets, product performance, inventory and barcode stock signals, ad performance, customer-care summary metrics, and CSV exports for Sales, Products, Inventory, Ads, and Returns. Storefront/session conversion tracking and scheduled reports remain future work. |
...
2. Complete the human desktop/mobile seller-flow visual walkthrough from `docs/seller-flow-test-runbook.md`; Phase 10F covered seeded idempotency, static route QA, builds, tests, and documented deferred visual checks in `docs/seller-flow-qa-results.md`.
3. Implement the next seller depth candidate: seller notification analytics, historical stock-ledger backfill/barcode scanner polish, public seller acquisition analytics/content management, bulk published-variant workflow polish, or scheduled seller report delivery.
